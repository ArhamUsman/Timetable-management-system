...
void FileHandler::read_all_files(vector <week_day> &weekdays){
    ...
    for (int i=0; i<5; i++){
        ...
        string filename;
        switch(i) {
            case 0: filename = file_monday; break;
            case 1: filename = file_tuesday; break;
            case 2: filename = file_wednesday; break;
            case 3: filename = file_thursday; break;
            case 4: filename = file_friday; break;
        }
        ...
        while (getline(file, line)) {
            ...
            for (const string &cell : cells) {
                ...
                vector<string> parts = split(cell, '/');
                if (parts.size() == 6) {
                    period_num = string_to_num(trimEnd(parts[0]));
                    time = trimEnd(parts[1]);
                    venue = trimEnd(parts[2]);
                    subject = trimEnd(parts[3]);
                    section = trimEnd(parts[4]);
                    teacher = trimEnd(parts[5]);

                    // Debug output
                    cout << "Adding slot: " << period_num << "|" << time << "|" 
                         << venue << "|" << subject << "|" << section << "|" 
                         << teacher << endl;

                    s = slot(period_num, Time(time), Venue(venue), 
