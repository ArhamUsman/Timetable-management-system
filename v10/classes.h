...
public:
    ...
    vector<slot> get_all_slots();
    void addSlot(slot& new_slot);
    vector<slot> get_slots(const Section& section, 
                              const vector<Subject>& subjects);
    week_day& operator=(const week_day& other);
    week_day& operator=(week_day&& other) noexcept;
};
